...
{literal}
	jQuery(document).ready(function($){
		$('#ps-cs').change(function() {
			var value = $('#ps-cs option:selected').val();
			$('input[name="pmethod"]').each(function(){
				$(this).attr('checked', false);
			});
			$('.ps-table').each(function(){
...
<table>
	<tr>
		...
		<td>
			<select id="ps-cs" style="margin:0">
...
<form action="" method="post" >
	...
	{foreach from=$payMethods item=country}
		<table id="{$country->getCode()}" class="ps-table" style="display:{if $country->getCode() == $defaultCountry}table{else}none{/if}">
